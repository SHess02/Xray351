...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check_stmt) {
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            $insert_sql = "INSERT INTO event (name, location, Admin_email, Alumni_email, datetime) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($insert_sql);
            if ($stmt) {
                $stmt->bind_param("sssss", $name, $location, $admin_email, $alumni_email, $formatted_datetime);
                if ($stmt->execute()) {
